...
def make_layers(in_channels, inner_channels, out_channels, num_class, stride, dropout, bias):
    input_channels = in_channels
    layers = []
    input_shortcut_flag = [False, False, True]
    shortcut_flag = [NO_SHORTCUT, NO_SHORTCUT, ADD_SHORTCUT]
    downsample_flag = [False, False, False]

    layers.append(CWConv(in_channels, inner_channels, 1, stride=1, padding=0, bias=bias,
                         num_class=num_class, dropout=dropout))
    layers.append(CWConv(inner_channels, inner_channels, 3, stride=stride, padding=1, bias=bias,
                            num_class=num_class, dropout=dropout))
    if in_channels != out_channels:
        out_channels = out_channels - in_channels
        shortcut_flag[2] = CONCAT_SHORTCUT
    layers.append(CWConv(inner_channels, out_channels, 1, stride=1, padding=0, bias=bias,
                            num_class=num_class, dropout=dropout))
...
class Resnet101(nn.Module):
    def __init__(self,
        ...
        # stage 1
        in_planes = planes[0]
        inner_channels = planes[0]
        out_channels = planes[0] * 4

        for i in range(3):
            layers, input_shortcut_flg, shortcut_flg, downsample_flg = make_layers(in_planes, inner_channels,
        ...
        inner_channels = planes[1]
        out_channels = planes[1] * 4
        # stage 2
        for i in range(4):
        ...
        inner_channels = planes[2]
        out_channels = planes[2] * 4
        # stage 3
        for i in range(23):
            stride = 2 if i == 0 else 1
            layers, input_shortcut_flg, shortcut_flg, downsample_flg = make_layers(in_planes, inner_channels,
        ...
        inner_channels = planes[3]
        out_channels = planes[3] * 4
        # stage 4
        for i in range(3):
            stride = 2 if i == 0 else 1
            layers, input_shortcut_flg, shortcut_flg, downsample_flg = make_layers(in_planes, inner_channels,
    ...
    def update_pipeline(self, dataloader, queue_size=5):
        ...
        threads = [threading.Thread(target=train_func,
                                    args=(self.layers[i], self.optimizers[i], len(dataloader), main_queues[i],
                                          main_queues[i+1], shortcut_queues[i], shortcut_queues[i+3], self.shortcut_flag[i]))
                   for i in range(len(self.layers))]
